...
完整分析见 **[docs/tsne-analysis.md](docs/tsne-analysis.md)**：为什么取 `model.model[-2]` 而不是检测头输出或更浅的层、混淆的那四类在二维平面上各自的坐标范围和重叠方式、长尾在压痕和折痕上留下的双重痕迹，以及咬折为什么只用 22 个样本就做到了 0.685。文末列了这张图不能证明的东西。
